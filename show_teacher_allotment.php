                    ...
                    <div class="container mt-5">
                        ...
                        <table class="table" border="2" id="myTable">
                            ...
                            <tbody>
                                ...
                                $sql = "SELECT teacher_id, subject_id, room_id FROM teacher_allotment";
                                ...
                                if ($result) {
                                    if ($result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            $teacher_sql = "SELECT name FROM teachers WHERE teacher_id = ?";
                                            $teacher_stmt = $conn->prepare($teacher_sql);
                                            $teacher_stmt->bind_param("i", $teacher_id);
                                            $teacher_stmt->execute();
                                            $teacher_result = $teacher_stmt->get_result();
                                            $teacher_name = ($teacher_result->num_rows > 0) ? $teacher_result->fetch_assoc()['name'] : '';

                                            // Fetch subject name
                                            $subject_id = $row['subject_id'];
                                            $subject_sql = "SELECT name FROM subjects WHERE subject_id = ?";
                                            $subject_stmt = $conn->prepare($subject_sql);
                                            $subject_stmt->bind_param("i", $subject_id);
                                            $subject_stmt->execute();
                                            $subject_result = $subject_stmt->get_result();
                                            $subject_name = ($subject_result->num_rows > 0) ? $subject_result->fetch_assoc()['name'] : '';

                                            // Fetch room name
                                            $room_id = $row['room_id'];
                                            $room_sql = "SELECT name FROM rooms WHERE room_id = ?";
                                            $room_stmt = $conn->prepare($room_sql);
                                            $room_stmt->bind_param("i", $room_id);
                                            $room_stmt->execute();
                                            $room_result = $room_stmt->get_result();
                                            $room_name = ($room_result->num_rows > 0) ? $room_result->fetch_assoc()['name'] : '';
                                        ...
                                        echo "<tr><td colspan='3'>No teacher allotments found</td></tr>";
                                    }
                                } else {
