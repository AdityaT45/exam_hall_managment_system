...
if (isset($_GET['exam_id'])) {
    ...
    $sql_exam = "SELECT exam_name, exam_date, start_time, end_time FROM exams WHERE id = ?";
    ...
    // 'id' parameter is not set, handle the error here
    echo "Error: Exam ID is not set.";
}
?>
                ...
                <div class="container-fluid adi">
                    ...
                    <table border="2" id="myTable">
                        <thead>
                            <tr>
                                ...
                                <th colspan="5">
                                    ...
                                    <p align="left">Date: <?php echo $exam_date; ?></p>

                                    <p align="right">Start Time: <?php echo $start_time; ?> &nbsp;&nbsp;&nbsp;&nbsp; End
                                        Time:
                                        <?php echo $end_time; ?>
                                    </p>
                                </th>
                        ...
                        <tbody>
                            ...
                            $sql_allotment = "SELECT allotments.*, classes.name AS class_name, subjects.name AS subject_name, rooms.name AS room_name, rooms.strength AS room_strength, exams.exam_name AS exam_name, students.role_number AS student_roll_no 
                            FROM allotments 
                            INNER JOIN classes ON allotments.class_id = classes.class_id 
                            INNER JOIN subjects ON allotments.subject_id = subjects.subject_id 
                            INNER JOIN rooms ON allotments.room_id = rooms.room_id 
                            INNER JOIN exams ON allotments.exam_id = exams.id
                            INNER JOIN students ON allotments.student_id = students.student_id
                            WHERE allotments.exam_id = $exam_id";
                            $result_allotment = $conn->query($sql_allotment);

                            if ($result_allotment->num_rows > 0) {
                                $allotments_by_room = array();

                                // Group allotments by room
                                while ($row_allotment = $result_allotment->fetch_assoc()) {
                                    $room_id = $row_allotment['room_id'];
                                    if (!isset($allotments_by_room[$room_id])) {
                                        $allotments_by_room[$room_id] = array(
                                            // 'id' => $row_allotment['id'],
                                            // 'class_name' => $row_allotment['class_name'],
                                            'subject_name' => $row_allotment['subject_name'],
                                            'room_name' => $row_allotment['room_name'],
                                            'room_strength' => $row_allotment['room_strength'],
                                            'exam_name' => $row_allotment['exam_name'],
                                            'student_roll_numbers' => array($row_allotment['student_roll_no'])
                                        );
                                    } else {
                                        $allotments_by_room[$room_id]['student_roll_numbers'][] = $row_allotment['student_roll_no'];
                                    }
                                }

                                // Output rows for each room
                                foreach ($allotments_by_room as $room_id => $allotment_data) {
                                    echo "<tr>";
                                    // echo "<td>" . $allotment_data['id'] . "</td>";
                                    // echo "<td>" . $allotment_data['class_name'] . "</td>";
                                    echo "<td>" . $allotment_data['subject_name'] . "</td>";
                                    echo "<td>" . $allotment_data['room_name'] . "</td>";
                                    echo "<td>" . $allotment_data['room_strength'] . "</td>";
                                    echo "<td>" . $allotment_data['exam_name'] . "</td>";
                                    echo "<td>" . implode(", ", $allotment_data['student_roll_numbers']) . "</td>";
                                    echo "</tr>";
                                }
                            } else {
                                echo "<tr><td colspan='7'>No allotments found</td></tr>";
                            }
                            ?>
                        </tbody>
                    </table>

                    <button class="btn btn-primary mt-5" type="button" id="printBtn">
                        Print Table
                    </button>
